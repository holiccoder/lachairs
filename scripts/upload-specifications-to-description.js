...
async function updateMagentoDescription(sku, description) {
  try {
...
async function main() {
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    const description = buildSpecsTable(p.specifications);
    ...
    const result = await updateMagentoDescription(sku, description);
  ...
  for (const file of productsFiles) {
    ...
    let changed = false;
    items.forEach(p => {
      const newDesc = buildSpecsTable(p.specifications);
      if (newDesc && newDesc !== p.description) {
        p.description = newDesc;
        if (p.magento) p.magento.description = newDesc;
        changed = true;
      }
    });
